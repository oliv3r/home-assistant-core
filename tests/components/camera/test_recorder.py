...
async def test_exclude_attributes(recorder_mock: recorder.Recorder, hass: HomeAssistant) -> None:
    ...
    states = await hass.async_add_executor_job(
        recorder.history.get_significant_states, hass, now, None, hass.states.async_entity_ids()
    )
    assert len(states) > 1
