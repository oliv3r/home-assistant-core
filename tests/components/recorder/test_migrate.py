...
def _get_native_states(hass, entity_id):
    with recorder.util.session_scope(hass=hass, read_only=True) as session:
        ...
        states = []
        for dbstate in session.query(recorder.db_schema.States).filter(recorder.db_schema.States.metadata_id == metadata_id):
...
def test_raise_if_exception_missing_str() -> None:
    ...
    with pytest.raises(ProgrammingError):
        recorder.migration.raise_if_exception_missing_str(programming_exc, ["not present"])
