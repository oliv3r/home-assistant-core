...
async def test_purge_old_states(
    ...
    with recorder.util.session_scope(hass=hass) as session:
        ...
        events = session.query(recorder.db_schema.Events).filter(recorder.db_schema.Events.event_type == "state_changed")
...
async def test_purge_old_states_encouters_database_corruption(
    ...
    with patch(
        ...
        await hass.services.async_call(recorder.DOMAIN,
                                       recorder.services.SERVICE_PURGE,
                                       {"keep_days": 0})
...
async def test_purge_old_events(
    ...
    with recorder.util.session_scope(hass=hass) as session:
        events = session.query(recorder.db_schema.Events).filter(
            recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids(TEST_EVENT_TYPES))
        )
...
async def test_purge_method(
    ...
    with recorder.util.session_scope(hass=hass) as session:
        ...
        statistics_runs = session.query(recorder.db_schema.StatisticsRuns).order_by(recorder.db_schema.StatisticsRuns.run_id)
        assert statistics_runs.count() == 7
...
async def test_purge_cutoff_date(
    ...
    with recorder.util.session_scope(hass=hass) as session:
        states = session.query(recorder.db_schema.States)
        state_attributes = session.query(recorder.db_schema.StateAttributes)
        assert states.filter(recorder.db_schema.States.state == "purge").count() == rows - 1
        assert states.filter(recorder.db_schema.States.state == "keep").count() == 1
        assert (
            state_attributes.outerjoin(
                recorder.db_schema.States,
                recorder.db_schema.StateAttributes.attributes_id == recorder.db_schema.States.attributes_id
            )
        ...
        assert (
            ...
            .filter(recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids(("PURGE",))))
            .count()
            == rows - 1
        )
        assert (
            session.query(recorder.db_schema.Events)
            .filter(recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids(("KEEP",))))
            .count()
            == 1
        )

    instance.queue_task(recorder.tasks.PurgeTask(cutoff, repack=False, apply_filter=False))
    await hass.async_block_till_done()
    await async_recorder_block_till_done(hass)
    await async_wait_purge_done(hass)
    ...
    instance.queue_task(recorder.tasks.PurgeTask(dt_util.utcnow(), repack=False, apply_filter=False))
    await async_recorder_block_till_done(hass)
    await async_wait_purge_done(hass)
...
async def test_purge_filtered_states(
    ...
    with recorder.util.session_scope(hass=hass) as session:
        ...
        events_keep = session.query(recorder.db_schema.Events).filter(
            recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids(("EVENT_KEEP",)))
        )
    ...
    with recorder.util.session_scope(hass=hass) as session:
        ...
        states_sensor_excluded = (
            ...
            .outerjoin(recorder.db_schema.StatesMeta,
                       recorder.db_schema.States.metadata_id == recorder.db_schema.StatesMeta.metadata_id)
            .filter(recorder.db_schema.StatesMeta.entity_id == "sensor.excluded")
        )
        assert states_sensor_excluded.count() == 0

        assert (
            session.query(recorder.db_schema.States).filter(recorder.db_schema.States.state_id == 72).first().old_state_id
            is None
        )
        assert (
            session.query(recorder.db_schema.States).filter(recorder.db_schema.States.state_id == 72).first().attributes_id
            == 71
        )
        assert (
            session.query(recorder.db_schema.States).filter(recorder.db_schema.States.state_id == 73).first().old_state_id
            is None
        )
        assert (
            session.query(recorder.db_schema.States).filter(recorder.db_schema.States.state_id == 73).first().attributes_id
            == 71
        )

        final_keep_state = session.query(recorder.db_schema.States).filter(recorder.db_schema.States.state_id == 74).first()
...
async def test_purge_filtered_events(
    ...
    with recorder.util.session_scope(hass=hass, read_only=True) as session:
        events_purge = session.query(recorder.db_schema.Events).filter(
            recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids(("EVENT_PURGE",)))
        )
        states = session.query(recorder.db_schema.States)
...
async def test_purge_filtered_events_state_changed(
    ...
    with recorder.util.session_scope(hass=hass) as session:
        ...
        events_purge = session.query(recorder.db_schema.Events).filter(
            recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids(("excluded_event",)))
        )
        states = session.query(recorder.db_schema.States)
    ...
    with recorder.util.session_scope(hass=hass) as session:
        ...
        assert (
            session.query(recorder.db_schema.States).filter(recorder.db_schema.States.state_id == 61).first().old_state_id
            is None
        )
        assert (
            session.query(recorder.db_schema.States).filter(recorder.db_schema.States.state_id == 62).first().old_state_id
            is None
        )
        assert (
            session.query(recorder.db_schema.States).filter(recorder.db_schema.States.state_id == 63).first().old_state_id
            == 62
...
async def test_purge_many_old_events(
    ...
    """Test deleting old events."""
    old_events_count = 5
    with patch.object(recorder.queries, "SQLITE_MAX_BIND_VARS", old_events_count), patch.object(
        recorder.purge, "SQLITE_MAX_BIND_VARS", old_events_count
    ):
        instance = await async_setup_recorder_instance(hass)
...
async def test_purge_old_events_purges_the_event_type_ids(
    ...
    def _insert_events():
        with recorder.util.session_scope(hass=hass) as session:
            event_type_test_auto_purge = recorder.db_schema.EventTypes(event_type="EVENT_TEST_AUTOPURGE")
            event_type_test_purge = recorder.db_schema.EventTypes(event_type="EVENT_TEST_PURGE")
            event_type_test = recorder.db_schema.EventTypes(event_type="EVENT_TEST")
            event_type_unused = recorder.db_schema.EventTypes(event_type="EVENT_TEST_UNUSED")
            session.add_all(
                (
...
async def test_purge_old_states_purges_the_state_metadata_ids(
    ...
    def _insert_states():
        with recorder.util.session_scope(hass=hass) as session:
            states_meta_sensor_one = recorder.db_schema.StatesMeta(entity_id="sensor.one")
            states_meta_sensor_two = recorder.db_schema.StatesMeta(entity_id="sensor.two")
            states_meta_sensor_three = recorder.db_schema.StatesMeta(entity_id="sensor.three")
            states_meta_sensor_unused = recorder.db_schema.StatesMeta(entity_id="sensor.unused")
            session.add_all(
                (
    ...
    with recorder.util.session_scope(hass=hass) as session:
        states = session.query(recorder.db_schema.States).where(recorder.db_schema.States.metadata_id.in_(test_metadata_ids))
