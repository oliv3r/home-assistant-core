...
def small_cache_size() -> None:
    ...
    with patch.object(recorder.table_managers.state_attributes, "CACHE_SIZE", 8), patch.object(
        recorder.table_managers.states_meta, "CACHE_SIZE", 8
    ):
        yield
...
async def test_saving_state(recorder_mock: recorder.Recorder, hass: HomeAssistant) -> None:
    ...
    with recorder.util.session_scope(hass=hass, read_only=True) as session:
        ...
        for db_state, db_state_attributes, states_meta in (
            ...
            .outerjoin(recorder.db_schema.StateAttributes,
                       recorder.db_schema.States.attributes_id == recorder.db_schema.StateAttributes.attributes_id)
...
@pytest.mark.parametrize(
    ...
    (
        (recorder.const.SupportedDialect.MYSQL, {"test_attr": 5, "test_attr_10": "silly\0stuff"}),
        (recorder.const.SupportedDialect.POSTGRESQL, {"test_attr": 5, "test_attr_10": "silly"}),
        (recorder.const.SupportedDialect.SQLITE, {"test_attr": 5, "test_attr_10": "silly\0stuff"}),
    ),
)
async def test_saving_state_with_nul(
    recorder_mock: recorder.Recorder, hass: HomeAssistant, dialect_name, expected_attributes
) -> None:
...
def test_saving_event(hass_recorder: Callable[..., HomeAssistant]) -> None:
    ...
    with recorder.util.session_scope(hass=hass, read_only=True) as session:
        for select_event, event_data, event_types in (
            ...
            .filter(recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids((event_type,))))
            .outerjoin(recorder.db_schema.EventTypes,
                       recorder.db_schema.Events.event_type_id == recorder.db_schema.EventTypes.event_type_id)
            .outerjoin(recorder.db_schema.EventData,
...
def test_setup_without_migration(hass_recorder: Callable[..., HomeAssistant]) -> None:
    ...
    hass = hass_recorder()
    assert recorder.get_instance(hass).schema_version == recorder.db_schema.SCHEMA_VERSION
...
def test_saving_state_and_removing_entity(
    ...
    with recorder.util.session_scope(hass=hass, read_only=True) as session:
        states = list(
            session.query(recorder.db_schema.StatesMeta.entity_id,
...
def test_saving_event_with_oversized_data(
    ...
    with recorder.util.session_scope(hass=hass, read_only=True) as session:
        for _, data, event_type in (
            session.query(recorder.db_schema.Events.event_id,
            ...
            .outerjoin(recorder.db_schema.EventTypes,
                       recorder.db_schema.Events.event_type_id == recorder.db_schema.EventTypes.event_type_id)
            .where(recorder.db_schema.EventTypes.event_type.in_(["test_event", "test_event_too_big"]))
        ):
...
def test_auto_statistics(hass_recorder: Callable[..., HomeAssistant], freezer) -> None:
    ...
    hass.bus.listen(
        recorder.const.EVENT_RECORDER_5MIN_STATISTICS_GENERATED, async_5min_stats_updated_listener
    )
    hass.bus.listen(
        recorder.const.EVENT_RECORDER_HOURLY_STATISTICS_GENERATED, async_hourly_stats_updated_listener
    )
...
def test_compile_missing_statistics(
    ...
    recorder_helper.async_initialize_recorder(hass)
    setup_component(hass, recorder.DOMAIN, {recorder.DOMAIN: {recorder.CONF_DB_URL: dburl}})
    hass.start()
    wait_recording_done(hass)
    wait_recording_done(hass)
...
def test_has_services(hass_recorder: Callable[..., HomeAssistant]) -> None:
    ...
    assert hass.services.has_service(recorder.DOMAIN, recorder.services.SERVICE_PURGE_ENTITIES)
...
async def test_database_corruption_while_running(
    ...
    assert await async_setup_component(
        hass, recorder.DOMAIN, {recorder.DOMAIN: {recorder.CONF_DB_URL: dburl, recorder.CONF_COMMIT_INTERVAL: 0}}
    )
    await hass.async_block_till_done()
    caplog.clear()

    original_start_time = recorder.get_instance(hass).recorder_runs_manager.recording_start
        ...
        with recorder.util.session_scope(hass=hass, read_only=True) as session:
            db_events = list(
                session.query(recorder.db_schema.Events).filter(
                    recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids(event_types))
                )
            )
...
def test_deduplication_event_data_inside_commit_interval(
    ...
    with recorder.util.session_scope(hass=hass, read_only=True) as session:
        ...
        events = list(
            session.query(recorder.db_schema.Events)
            .filter(recorder.db_schema.Events.event_type_id.in_(recorder.queries.select_event_type_ids(event_types)))
            .outerjoin(recorder.db_schema.EventTypes,
                       recorder.db_schema.Events.event_type_id == recorder.db_schema.EventTypes.event_type_id)
            .outerjoin(recorder.db_schema.EventData,
                       recorder.db_schema.Events.data_id == recorder.db_schema.EventData.data_id)
...
async def test_disable_echo(
    ...
    with patch(
        ...
        await async_setup_component(hass, recorder.DOMAIN, {recorder.DOMAIN: {recorder.CONF_DB_URL: db_url}})
...
async def test_mysql_missing_utf8mb4(
    ...
    with patch(
        ...
        await async_setup_component(hass, recorder.DOMAIN, {recorder.DOMAIN: {recorder.CONF_DB_URL: config_url}})
        create_engine_mock.assert_called_once()
...
async def test_clean_shutdown_when_recorder_thread_raises_during_initialize_database(
    ...
    with patch.object(recorder.migration, "initialize_database", side_effect=Exception), patch(
...
async def test_clean_shutdown_when_recorder_thread_raises_during_validate_db_schema(
    ...
    with patch.object(recorder.migration, "validate_db_schema", side_effect=Exception), patch(
        "homeassistant.components.recorder.ALLOW_IN_MEMORY_DB", True
    ):
        if recorder.DOMAIN not in hass.data:
            recorder_helper.async_initialize_recorder(hass)
            ...
            for select_event, event_types in (
                ...
                .filter(
                    recorder.db_schema.Events.event_type_id.in_(
                        recorder.queries.select_event_type_ids(("fake_event", "after_final_write"))
                    )
                )
                .outerjoin(
                    recorder.db_schema.EventTypes,
