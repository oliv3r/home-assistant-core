...
class Events(Base):  # type: ignore[misc,valid-type]
    ...
    def from_event(
        ...
        return Events(
            ...
            event_data=recorder.const.JSON_DUMP(event.data) if event_data is UNDEFINED else event_data,
...
class StateAttributes(Base):  # type: ignore[misc,valid-type]
    ...
    def from_event(event: Event) -> StateAttributes:
        ...
        dbstate = StateAttributes(
            shared_attrs="{}" if state is None else recorder.const.JSON_DUMP(state.attributes)
        )
    ...
    def shared_attrs_from_event(
        ...
        exclude_attrs = (
            exclude_attrs_by_domain.get(domain, set()) | recorder.const.ALL_DOMAIN_EXCLUDE_ATTRS
        )
