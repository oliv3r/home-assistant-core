...
def test_setup_connection_for_dialect_mysql(mysql_version) -> None:
    ...
    recorder.util.setup_connection_for_dialect(instance_mock, "mysql", dbapi_connection, True)
...
async def test_issue_for_mariadb_with_MDEV_25020(
    ...
    issue = registry.async_get_issue(recorder.const.DOMAIN, "maria_db_range_index_regression")
...
def test_is_second_sunday() -> None:
    ...
    assert recorder.util.is_second_sunday(datetime(2022, 1, 9, 0, 0, 0, tzinfo=dt_util.UTC)) is True
    assert recorder.util.is_second_sunday(datetime(2022, 2, 13, 0, 0, 0, tzinfo=dt_util.UTC)) is True
    assert recorder.util.is_second_sunday(datetime(2022, 3, 13, 0, 0, 0, tzinfo=dt_util.UTC)) is True
    assert recorder.util.is_second_sunday(datetime(2022, 4, 10, 0, 0, 0, tzinfo=dt_util.UTC)) is True
    assert recorder.util.is_second_sunday(datetime(2022, 5, 8, 0, 0, 0, tzinfo=dt_util.UTC)) is True

    assert recorder.util.is_second_sunday(datetime(2022, 1, 10, 0, 0, 0, tzinfo=dt_util.UTC)) is False
...
def test_execute_stmt_lambda_element(
    ...
    with recorder.util.session_scope(hass=hass) as session:
        ...
        rows = recorder.util.execute_stmt_lambda_element(session, stmt, now, one_week_from_now)
...
async def test_resolve_period(hass: HomeAssistant) -> None:
    ...
    start_t, end_t = recorder.util.resolve_period({"calendar": {"period": "hour", "offset": -1}})
    ...
    start_t, end_t = recorder.util.resolve_period({"calendar": {"period": "day", "offset": -1}})
    ...
    start_t, end_t = recorder.util.resolve_period({"calendar": {"period": "week", "offset": -1}})
    ...
    start_t, end_t = recorder.util.resolve_period({"calendar": {"period": "month", "offset": -1}})
    ...
    start_t, end_t = recorder.util.resolve_period({"calendar": {"period": "year", "offset": -1}})
    ...
    assert recorder.util.resolve_period({"fixed_period": {"end_time": now}}) == (None, now)

    assert recorder.util.resolve_period({"fixed_period": {"start_time": now}}) == (now, None)

    assert recorder.util.resolve_period({"fixed_period": {"end_time": now, "start_time": now}}) == (
        now,
        now,
    )
