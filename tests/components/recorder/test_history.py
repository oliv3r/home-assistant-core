...
def test_get_significant_states(hass_recorder: Callable[..., HomeAssistant]) -> None:
    ...
    hist = recorder.history.get_significant_states(hass, zero, four, entity_ids=list(states))
...
def test_get_significant_states_entity_id(
    ...
    hist = recorder.history.get_significant_states(hass, zero, four, ["media_player.test"])
    assert_dict_of_states_equal_without_context_and_last_changed(states, hist)
    ...
    def _fetch_native_states() -> list[State]:
        with recorder.util.session_scope(hass=hass, read_only=True) as session:
            ...
            db_state_attributes = {
                ...
                for state_attributes in session.query(recorder.db_schema.StateAttributes)
            }
...
def test_get_significant_states_with_non_existent_entity_ids_returns_empty(
    ...
    hass = hass_recorder()
    now = dt_util.utcnow()
    assert recorder.history.get_significant_states(hass, now, None, ["nonexistent.entity"]) == {}
...
def test_state_changes_during_period_with_non_existent_entity_ids_returns_empty(
    ...
    now = dt_util.utcnow()
    assert (
        recorder.history.state_changes_during_period(hass, now, None, "nonexistent.entity") == {}
    )
