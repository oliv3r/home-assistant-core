...
def test_converters_align_with_sensor() -> None:
    ...
    for converter in UNIT_CONVERTERS.values():
        assert converter in recorder.statistics.STATISTIC_UNIT_TO_UNIT_CONVERTER.values()
...
def test_compile_hourly_statistics(hass_recorder: Callable[..., HomeAssistant]) -> None:
    ...
    metadata = recorder.statistics.get_metadata(hass, statistic_ids={"sensor.test1", "sensor.test2"})
    ...
    expected_1 = {
        ...
        "end": recorder.models.process_timestamp(zero + timedelta(minutes=5)).timestamp(),
    ...
    expected_2 = {
        ...
        "end": recorder.models.process_timestamp(four + timedelta(minutes=5)).timestamp(),
...
def test_compile_periodic_statistics_exception(
    ...
    expected_1 = {
        ...
        "end": recorder.models.process_timestamp(now + timedelta(minutes=5)).timestamp(),
        "mean": None,
        "min": None,
        "max": None,
        "last_reset": None,
    ...
    expected_2 = {
        "start": recorder.models.process_timestamp(now + timedelta(minutes=5)).timestamp(),
        "end": recorder.models.process_timestamp(now + timedelta(minutes=10)).timestamp(),
        "mean": None,
        "min": None,
        "max": None,
        "last_reset": None,
...
@pytest.mark.parametrize(
    ...
    (
        ("test", "test:total_energy_import", recorder.statistics.async_add_external_statistics),
        ("recorder", "sensor.total_energy_import", recorder.statistics.async_import_statistics),
    ),
)
        ...
        statistic_id: [
            {
                ...
                "end": recorder.models.process_timestamp(period1 + timedelta(hours=1)).timestamp(),
                "last_reset": recorder.models.datetime_to_timestamp_or_none(last_reset_utc),
                "state": pytest.approx(0.0),
                "sum": pytest.approx(2.0),
            },
            {
                "start": recorder.models.process_timestamp(period2).timestamp(),
                "end": recorder.models.process_timestamp(period2 + timedelta(hours=1)).timestamp(),
                "last_reset": recorder.models.datetime_to_timestamp_or_none(last_reset_utc),
...
def test_external_statistics_errors(
    ...
    with pytest.raises(HomeAssistantError):
        recorder.statistics.async_add_external_statistics(hass, external_metadata, (external_statistics,))
    wait_recording_done(hass)
    assert statistics_during_period(hass, zero, period="hour") == {}
    assert recorder.statistics.list_statistic_ids(hass) == []
    assert recorder.statistics.get_metadata(hass, statistic_ids={"sensor.total_energy_import"}) == {}
    ...
    assert statistics_during_period(hass, zero, period="hour") == {}
    assert recorder.statistics.list_statistic_ids(hass) == []
    assert recorder.statistics.get_metadata(hass, statistic_ids={"test:total_energy_import"}) == {}
...
def test_import_statistics_errors(
    ...
    external_statistics = {**_external_statistics}
    with pytest.raises(HomeAssistantError):
        recorder.statistics.async_import_statistics(hass, external_metadata, (external_statistics,))
    wait_recording_done(hass)
    assert statistics_during_period(hass, zero, period="hour") == {}
    assert recorder.statistics.list_statistic_ids(hass) == []
    assert recorder.statistics.get_metadata(hass, statistic_ids={"test:total_energy_import"}) == {}
...
def test_cache_key_for_generate_statistics_during_period_stmt() -> None:
    ...
    stmt = recorder.statistics._generate_statistics_during_period_stmt(
        dt_util.utcnow(), dt_util.utcnow(), [0], recorder.db_schema.StatisticsShortTerm, set()
    )
...
def test_cache_key_for_generate_get_metadata_stmt() -> None:
    ...
    stmt_mean = recorder.table_managers.statistics_meta._generate_get_metadata_stmt([0], "mean")
    stmt_mean2 = recorder.table_managers.statistics_meta._generate_get_metadata_stmt([1], "mean")
    stmt_sum = recorder.table_managers.statistics_meta._generate_get_metadata_stmt([0], "sum")
...
def test_cache_key_for_generate_max_mean_min_statistic_in_sub_period_stmt() -> None:
    ...
    columns = select(recorder.db_schema.StatisticsShortTerm.metadata_id, recorder.db_schema.StatisticsShortTerm.start_ts)
...
def test_cache_key_for_generate_statistics_at_time_stmt() -> None:
    ...
    stmt = recorder.statistics._generate_statistics_at_time_stmt(recorder.db_schema.StatisticsShortTerm, {0}, 0.0, set())
    cache_key_1 = stmt._generate_cache_key()
    stmt2 = recorder.statistics._generate_statistics_at_time_stmt(recorder.db_schema.StatisticsShortTerm, {0}, 0.0, set())
    cache_key_2 = stmt2._generate_cache_key()
    assert cache_key_1 == cache_key_2
