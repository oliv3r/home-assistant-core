...
def test_from_event_to_db_event() -> None:
    ...
    db_event.event_data = recorder.db_schema.EventData.shared_data_bytes_from_event(event, dialect)
...
def test_from_event_to_db_state() -> None:
    ...
    assert state.as_dict() == recorder.db_schema.States.from_event(event).to_native().as_dict()
...
def test_from_event_to_db_state_attributes() -> None:
    ...
    db_attrs.shared_attrs = recorder.db_schema.StateAttributes.shared_attrs_bytes_from_event(
        event, {}, {}, dialect
    )
...
def test_repr() -> None:
    ...
    assert "2016-07-09 11:00:00+00:00" in repr(recorder.db_schema.States.from_event(event))
    assert "2016-07-09 11:00:00+00:00" in repr(recorder.db_schema.Events.from_event(event))
...
async def test_lazy_state_handles_include_json(
    ...
    assert recorder.models.LazyState(row, {}, None, row.entity_id, "", 1, False).attributes == {}
    assert "Error converting row to state attributes" in caplog.text
...
def test_bytes_to_ulid_or_none(caplog: pytest.LogCaptureFixture) -> None:
    ...
    assert (
        recorder.models.bytes_to_ulid_or_none(b"\x01w\xaf\xf9w\xe5\xf8~\x1f\x87\xe1\xf8~\x1f\x87\xe1")
