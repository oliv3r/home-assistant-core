...
def convert_pending_states_to_meta(instance: recorder.Recorder,
    ...
    for state in states:
        ...
        if entity_id not in states_meta_objects:
            states_meta_objects[entity_id] = recorder.db_schema.StatesMeta(entity_id=entity_id)
        state.states_meta_rel = states_meta_objects[entity_id]


def convert_pending_events_to_event_types(instance: recorder.Recorder,
                                          session: Session) -> None:
    ...
    for event in events:
        ...
        if event_type not in event_types_objects:
            event_types_objects[event_type] = recorder.db_schema.EventTypes(event_type=event_type)
