...
async def test_compile_hourly_sum_statistics_amount(
    ...
    await async_wait_recording_done(hass)
    statistic_ids = await hass.async_add_executor_job(recorder.statistics.list_statistic_ids, hass)
    assert statistic_ids == [
        {
            "statistic_id": "sensor.test1",
        ...
        "sensor.test1": [
            {
                ...
                "mean": None,
                "min": None,
                "last_reset": recorder.models.process_timestamp(dt_util.as_local(one)).timestamp(),
                "state": pytest.approx(factor * seq[7]),
                "sum": pytest.approx(factor * (sum(seq) - seq[0])),
            },
            {
                "start": recorder.models.process_timestamp(zero + timedelta(minutes=5)).timestamp(),
                "end": recorder.models.process_timestamp(zero + timedelta(minutes=10)).timestamp(),
                "max": None,
                "mean": None,
                "min": None,
                "last_reset": recorder.models.process_timestamp(dt_util.as_local(two)).timestamp(),
                "state": pytest.approx(factor * seq[7]),
...
def test_list_statistic_ids(
    ...
    for stat_type in ["mean", "sum", "dogs"]:
        statistic_ids = recorder.statistics.list_statistic_ids(hass, statistic_type=stat_type)
...
def test_compile_hourly_statistics_convert_units_1(
    ...
    assert stats == {
        "sensor.test1": [
            ...
            {
                "start": recorder.models.process_timestamp(zero + timedelta(minutes=10)).timestamp(),
                "end": recorder.models.process_timestamp(zero + timedelta(minutes=15)).timestamp(),
                "mean": pytest.approx(mean),
                "min": pytest.approx(min),
                "max": pytest.approx(max),
                "last_reset": None,
        ...
        "sensor.test1": [
            {
                ...
                "end": recorder.models.process_timestamp(zero + timedelta(seconds=30 * 15)).timestamp(),
                "mean": pytest.approx(mean),
                "min": pytest.approx(min),
                "max": pytest.approx(max),
                "last_reset": None,
...
def test_compile_hourly_statistics_changing_device_class_1(
    ...
    assert stats == {
        "sensor.test1": [
            ...
            {
                "start": recorder.models.process_timestamp(zero + timedelta(minutes=20)).timestamp(),
                "end": recorder.models.process_timestamp(zero + timedelta(minutes=25)).timestamp(),
                "mean": pytest.approx(mean2),
                "min": pytest.approx(min),
                "max": pytest.approx(max),
                "last_reset": None,
