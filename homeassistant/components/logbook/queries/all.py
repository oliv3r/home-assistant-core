...
def all_stmt(
    ...
    if context_id_bin is not None:
        stmt += lambda s: s.where(recorder.db_schema.Events.context_id_bin == context_id_bin).union_all(
...
def _apply_all_hints(sel: Select) -> Select:
    ...
    return sel.with_hint(
        recorder.db_schema.States, f"FORCE INDEX ({recorder.db_schema.LAST_UPDATED_INDEX_TS})", dialect_name="mysql"
    ).with_hint(
        recorder.db_schema.States, f"FORCE INDEX ({recorder.db_schema.LAST_UPDATED_INDEX_TS})", dialect_name="mariadb"
    )
