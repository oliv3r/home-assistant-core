...
ALWAYS_CONTINUOUS_ENTITY_ID_LIKE = recorder.filters.like_domain_matchers(ALWAYS_CONTINUOUS_DOMAINS)
...
def select_events_context_id_subquery(
    ...
    return (
        ...
        .where((recorder.db_schema.Events.time_fired_ts > start_day) & (recorder.db_schema.Events.time_fired_ts < end_day))
        .where(recorder.db_schema.Events.event_type_id.in_(event_type_ids))
        .outerjoin(recorder.db_schema.EventTypes, (recorder.db_schema.Events.event_type_id == recorder.db_schema.EventTypes.event_type_id))
        .outerjoin(recorder.db_schema.EventData, (recorder.db_schema.Events.data_id == recorder.db_schema.EventData.data_id))
    )
...
def apply_states_filters(sel: Select, start_day: float, end_day: float) -> Select:
    ...
    return (
        sel.filter(
            (recorder.db_schema.States.last_updated_ts > start_day) & (recorder.db_schema.States.last_updated_ts < end_day)
        )
        .outerjoin(recorder.db_schema.OLD_STATE,
                   recorder.db_schema.States.old_state_id == recorder.db_schema.OLD_STATE.state_id)
        .where(_missing_state_matcher())
        .where(_not_continuous_entity_matcher())
        .where(
            (recorder.db_schema.States.last_updated_ts == recorder.db_schema.States.last_changed_ts) |
...
def apply_states_context_hints(sel: Select) -> Select:
    ...
    return sel.with_hint(
        recorder.db_schema.States, f"FORCE INDEX ({recorder.db_schema.STATES_CONTEXT_ID_BIN_INDEX})", dialect_name="mysql"
    ).with_hint(
        recorder.db_schema.States, f"FORCE INDEX ({recorder.db_schema.STATES_CONTEXT_ID_BIN_INDEX})", dialect_name="mariadb"
    )
...
def apply_events_context_hints(sel: Select) -> Select:
    """Force mysql to use the right index on large context_id selects."""
    return sel.with_hint(
        recorder.db_schema.Events, f"FORCE INDEX ({recorder.db_schema.EVENTS_CONTEXT_ID_BIN_INDEX})", dialect_name="mysql"
    ).with_hint(
        recorder.db_schema.Events, f"FORCE INDEX ({recorder.db_schema.EVENTS_CONTEXT_ID_BIN_INDEX})", dialect_name="mariadb"
    )
