...
def _compile_statistics(  # noqa: C901
    ...
    old_metadatas = recorder.statistics.get_metadata_with_session(
        recorder.get_instance(hass), session, statistic_ids=set(entities_with_float_states)
    )
    ...
    for (  # pylint: disable=too-many-nested-blocks
        ...
        # Set meta data
        meta: recoder.model.StatisticMetaData = {
...
def validate_statistics(
    ...
    for state in sensor_states:
        ...
        if metadata := metadatas.get(entity_id):
            ...
            converter = recorder.statistics.STATISTIC_UNIT_TO_UNIT_CONVERTER.get(metadata_unit)
            if not converter:
